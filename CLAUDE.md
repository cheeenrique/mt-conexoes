...
Aplicação real, rodando. Next.js 16 (App Router) + React 19 + Prisma + Postgres. `pnpm test`, `pnpm test:integration`, `pnpm lint`, `pnpm typecheck` e `pnpm build` rodam contra código real; `pnpm dev` sobe o painel. Conferido em 20/09/2026: 710 testes unitários e 390 de integração passando (119 arquivos, 22 migrations).

**Existe** — as features `auth`, `charges`, `customers`, `dunning`, `leads`, `messaging`, `plans`, `reports`, `settings`, `subscriptions`, `suppliers` em `src/features/`, e as telas Início, Clientes (ficha em gaveta, com modo edição), Cobranças, Mensagens, Réguas (mestre-detalhe), Leads, Relatórios, Fornecedores, Planos, Ajustes (abas Negócio e Canais) e Conta. A ficha do cliente ganhou a seção **Alterações**, alimentada por `financial_events`: toda mutação de assinatura, cobrança e pagamento grava antes/depois no mesmo commit da transação que a executa, e anonimizar o cliente limpa o `reason` de cada evento — texto livre do operador, único campo do log com risco de dado pessoal — mantendo valores, datas e ids. Crons: `charges-mark-overdue`, `dunning-evaluate`, `messages-dispatch`, `ping`. Rotas públicas: `POST /api/leads`, os webhooks `evolution` e `meta-cloud`, `/api/health`.

Quatro coisas que a estrutura não deixa óbvias: **assinatura não tem rota própria** — vive na ficha do cliente (`/customers/[id]`); **não existe `/channels`** — canal é a aba `Canais` de `/settings?aba=canais`; **a régua está em `/dunning`**, não mais em `/regua`; e **`/conta` é item de menu que abre em gaveta**, não link — a rota segue existindo para link direto, mas o caminho normal é o botão `Conta` da barra lateral (`features/auth/components/account-drawer.tsx`). A **importação da planilha** tem dois caminhos pra mesma regra: a tela, em rota própria (`/customers/import`, alcançada por Clientes → **Importar planilha**) com **duas etapas** — escolher fornecedor e arquivo, depois conferir o plano (novas · já existem · recusadas, soma, margem média, tabela linha a linha) **sem gravar nada**; só o botão final grava, e o arquivo escolhido é reenviado do navegador na confirmação, sem ficar guardado no servidor entre as duas etapas — e o CLI (`pnpm import:customers <arquivo.xlsx> "<Fornecedor>"`, que segue existindo e faz upsert do fornecedor pelo nome). Os três pontos de entrada (prévia, confirmação, CLI) chamam `app/(app)/customers/customer-import.ts` — nenhuma regra de linha duplicada. **Toda linha importada abre também a primeira `Charge`** no vencimento que veio da planilha (período = um ciclo para trás, `periodStartForDue`): sem ela o cliente ficava `ACTIVE` e sem cobrança nenhuma — fora de `/charges`, fora dos chips “Vence hoje”/“Em atraso”, fora da régua, e sem `chargeId` para registrar pagamento. A base importada antes dessa correção se resolve com `pnpm backfill:imported-charges` (sem `--apply` só lista).
...
- **"Ativar sem descartar a revisão" não dispara retroativo.** `UNIQUE(chargeId, stepId)` impede reprocessar o par, então nenhuma daquelas mensagens sai. O rótulo do botão já é honesto (o handoff chamava de "Enviar todas"); o comportamento certo é decisão de produto em aberto. ⚠️ O caso que mais doía — cobrança vencida que passou por baixo da escada e nunca recebeu nada — deixou de depender disso: o degrau de recuperação (§Régua — travas) pega essas cobranças na passada seguinte. O que continua em aberto é só o retroativo dos degraus **intermediários**, que ninguém pediu.
...
- **Baixa do restante** (`writeOffRemaining`) é a única saída para "o cliente pagou parte e não vai pagar o resto": o que falta vira `discountCents` na própria cobrança, que fecha `PAID` com o valor recebido. Não reescreve pagamento, não mexe em `costCents`, e o faturado cai para o que foi de fato acordado. Sem pagamento nenhum não é baixa, é cancelamento.
- **Cobrança em aberto sem pagamento segue o vencimento da assinatura.** Editar `nextDueAt` na ficha move `charge.dueAt` junto (`alignOpenChargeDueAt`) e cancela as mensagens pendentes daquela cobrança — sem isso a ficha e a lista mostram duas datas para o mesmo conceito e a régua cobra o vencimento velho. Valor e custo continuam congelados na emissão.
- **Trocar de plano pergunta sobre a cobrança em aberto; reajuste não.** Sem realinhar, valor e custo da `Charge` continuam congelados na emissão — trocar o plano sozinho **não** os reescreve. O que mudou é o sinal: quando o `planId` muda e existe cobrança em aberto **sem pagamento** num valor diferente, a gravação devolve `staleOpenCharge` (`findOpenChargeWithOldPlanAmount`) e a tela oferece o realinhamento, nos dois caminhos — a ficha e a coluna "Plano" da tabela. Quem aplica é `realignChargeToSubscription`, que também cancela as mensagens `PENDING` daquela cobrança (`amount_changed`): o valor vai congelado no corpo desde a avaliação da régua, então realinhar sem cancelar manda o preço velho no WhatsApp. Aceitar o realinhamento traz **preço e custo** da assinatura para a cobrança — não só o preço, e isso é o certo: `realignChargeToSubscription` recusa cobrança com qualquer pagamento (`ChargeHasPaymentError`), então só toca cobrança **não paga**. O plano é pré-pago — o cliente paga para usar e nunca fica em débito —, então cobrança sem pagamento é orçamento do ciclo que ainda não começou, e o custo dela é o do plano novo. `costCents` congelado protege a cobrança **paga**, cujo período já foi entregue, de mudar de margem porque o fornecedor reajustou depois; recotar um ciclo que ninguém usou não é esse caso. ⚠️ A trava que sustenta isso é a recusa de cobrança com pagamento. Relaxá-la — está previsto na fatia 2, para permitir troca de plano com pagamento parcial — **não pode** levar junto a reescrita de `costCents`: ali existe período parcialmente entregue, e aí sim a margem mudaria retroativamente. Quem mexer naquele guard preserva o custo congelado. Editar só o preço, sem trocar de plano, **não** pergunta nada: reajuste combinado vale do próximo ciclo e ali a cobrança corrente está certa — perguntar toda vez treina o operador a clicar "sim" sem ler. Sem esse sinal, o cliente que saiu do trimestral de R$ 90,00 para o mensal de R$ 35,00 continuava com R$ 90,00 em `/charges`, no diálogo de pagamento e no WhatsApp; pagava R$ 35,00, virava `PARTIALLY_PAID` e a régua seguia cobrando. **O diálogo de pagamento também pergunta**, porque a ficha só pergunta na hora da troca: cobrança sem pagamento cujo valor difere do plano de hoje (`hasOldPlanAmount`) mostra "valor desta cobrança" e "valor do plano atual", nenhum marcado, e escolher o plano realinha no mesmo commit do pagamento (`realignChargeWithinTx`), com o saldo conferido contra o valor novo. Sem isso, a troca anterior ao sinal ou recusada na ficha travava — o mensal de R$ 35,00 que virou trimestral de R$ 75,00 batia em "maior que o saldo devedor", e aceitar os R$ 35,00 abria o ciclo trimestral pelo preço do mensal.
- **Assinatura de cortesia não gera cobrança nenhuma.** Preço zero **é** a marcação (`isCourtesySubscription` em `core/billing.ts`) — não existe campo separado, porque `isCourtesy: true` com `priceCents: 3000` é um estado que alguém cria sem querer. Os quatro pontos que emitem `Charge` checam: criação da assinatura, `openNextCycle`, importação e o backfill. Sem cobrança, a cortesia fica fora de `/charges`, dos chips de vencimento e da régua — que é o ponto. ⚠️ Cortesia **não** é isenção de um ciclo: "este mês não cobro" é `discountType`/`discountValue`, que mantém a cobrança existindo com valor zerado. Antes desta regra a cobrança de R$ 0 nascia `PAID` (`deriveChargeStatus`: `0 >= 0`), nunca passava por registrar pagamento e por isso nunca chamava `openNextCycle`: emitia uma vez e parava para sempre.

**Data e fuso**
- Banco em UTC; vencimento e corte de relatório são conceitos **locais** (`Settings.timezone`).
- Vencimento é `23:59:59` local.
- **Vencimento do próximo ciclo = âncora + duração do ciclo**, onde a âncora é **a data mais tarde entre o vencimento em aberto e o dia do pagamento**. Pagou antes do vencimento → conta do vencimento (vence 10, pagou 05 → 10 do mês seguinte); pagou depois → conta do pagamento (vence 10, pagou 12 → 12 do mês seguinte). Regra do operador, 11/09/2026: contar sempre do pagamento fazia quem paga adiantado **perder** os dias adiantados, e o vencimento andava para trás mês a mês (dia 10 → dia 5 → dia 1) em toda a base que paga antes. Não existe dia fixo guardado na assinatura: a âncora sai sempre desses dois valores.
- O diálogo de pagamento **sugere** esse vencimento num campo editável. Data combinada caso a caso é decisão do operador; campo vazio cai na regra. O que ele escolher é o que grava.
- Fim de mês aplica clamp sobre o dia da âncora, sem lembrar de um dia "desejado" entre ciclos: âncora 31/01 → vence 28/02 (fevereiro não tem 31); âncora 28/02 → vence 28/03, não 31/03. O dia 31 só reaparece se a âncora cair de novo no dia 31 **e** o mês alvo tiver 31 dias — nunca é automático: âncora 31/07, mensal → agosto tem 31 dias → vence 31/08. Cobrança nova só nasce em dois momentos: criação da assinatura (`startedAt + ciclo`) e quitação da cobrança do ciclo — pagamento total, ou baixa do restante, que conta do **último** pagamento. Pagamento parcial não gera cobrança nova. Quitar também religa a assinatura que a régua tinha suspendido (`SUSPENDED → ACTIVE`); pagamento parcial não religa.
...
- **O último degrau de mensagem casa por "a partir de", não por dia exato.** Os outros degraus casam `daysFromDue === offsetDays` — é o que faz a escada ser escada. O último `SEND_MESSAGE` ativo (`selectStepsForCharge` em `core/dunning-rules.ts`) pega também quem já passou dele. Sem isso, cobrança que passa por baixo da escada — envio pausado, canal caído, cron que falhou — nunca mais é tocada: aconteceu na base real, 8 cobranças vencidas sem nenhuma mensagem, uma há 30 dias. Dispara uma vez só por cobrança, garantido pelo `UNIQUE(chargeId, stepId)`, não por `if`. ⚠️ `SUSPEND` **nunca** é o degrau de recuperação, mesmo sendo o de maior offset: "a partir de" ali cortaria acesso em lote de toda a base atrasada de uma vez.
- **Mensagem que morre `stale` devolve o passo para a régua.** O cancelamento por T8 apaga a `DunningExecution` na mesma transação: ela afirma "este passo já foi processado", e uma mensagem que nunca tocou o provider foi planejada, não executada. Sem isso o degrau de recuperação dispara uma vez, morre numa pausa longa, e a cobrança fica órfã de novo — com o `UNIQUE` segurando a porta. Envio bem-sucedido nunca passa por esse caminho: histórico de envio não se apaga.
