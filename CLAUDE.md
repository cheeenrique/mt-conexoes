...
Aplicação real, rodando. Next.js 16 (App Router) + React 19 + Prisma + Postgres. `pnpm test`, `pnpm test:integration`, `pnpm lint`, `pnpm typecheck` e `pnpm build` rodam contra código real; `pnpm dev` sobe o painel. Conferido em 25/08/2026: 536 testes unitários e 271 de integração passando (94 arquivos, 18 migrations).
...
Quatro coisas que a estrutura não deixa óbvias: **assinatura não tem rota própria** — vive na ficha do cliente (`/customers/[id]`); **não existe `/channels`** — canal é a aba `Canais` de `/settings?aba=canais`; **a régua está em `/dunning`**, não mais em `/regua`; e **`/conta` é item de menu que abre em gaveta**, não link — a rota segue existindo para link direto, mas o caminho normal é o botão `Conta` da barra lateral (`features/auth/components/account-drawer.tsx`). A **importação da planilha** tem dois caminhos pra mesma regra: a tela (Clientes → **Importar planilha**, um arquivo por fornecedor já cadastrado, com relatório de importadas/puladas/recusadas na hora) e o CLI (`pnpm import:customers <arquivo.xlsx> "<Fornecedor>"`, que segue existindo e faz upsert do fornecedor pelo nome). Os dois chamam `app/(app)/customers/customer-import.ts` — nenhuma regra de linha duplicada entre tela e script.
