...
describe('dispatchPendingMessages', () => {
  ...
  it('respeita o lote derivado da capability do canal (Evolution 1/min → 2) e a ordem de criação', async () => {
    await seedActiveDefaultChannel();
    const customer = await seedCustomer();
    ...
    // `wait` instantâneo: só interessa que o jitter foi *chamado* entre os envios do lote,
    // não esperar de verdade — a ~60s de intervalo, esperar seria 1min real por teste.
    const result = await dispatchPendingMessages(IN_HOURS_NOW, Math.random, async () => {});

    // O lote encolheu junto com o ritmo anti-banimento (adapter Evolution, 14/09/2026):
    // 2 por passada, o resto espera a próxima. Com passada a cada 15min isso dá 88/dia.
  ...
  it('aplica jitter entre envios reais ao provider, mas não depois do último da leva', async () => {
    await seedActiveDefaultChannel();
    const customer = await seedCustomer();
    // 2 = o lote inteiro do Evolution a 1/min. Mais que isso não sai nesta passada,
    // e o teste mediria o corte do lote em vez do jitter.
    for (let i = 0; i < 2; i++) {
      await seedPendingMessage(customer.id, {
        createdAt: new Date(IN_HOURS_NOW.getTime() + i * 1000),
        scheduledDate: new Date(Date.UTC(2026, 7, 8 + i)),
    ...
    expect(result.sent).toBe(2);
    // 2 envios reais → 1 atraso entre eles, nenhum depois do último.
    expect(wait).toHaveBeenCalledTimes(1);
    expect(wait).toHaveBeenCalledWith(60_000); // 60_000 / 1 (rateLimitPerMinute do Evolution), sem jitter (random=0.5)
  });
